...
class KiteApp:
    ...
    def baskets(self):
        baskets = self.session.get(f"{self.root_url}/orders/baskets", headers=self.headers).json()["data"]
        return baskets
    ...
    def place_order(self, variety, exchange, tradingsymbol, transaction_type, quantity, product, order_type, price=None,
        ...
        for k in list(params.keys()):
            if params[k] is None:
                del params[k]
        str = f"{self.root_url}/orders/{variety}"
        print(str)
        order_id = self.session.post(f"{self.root_url}/orders/{variety}",data=params, headers=self.headers).json()
        return order_id
