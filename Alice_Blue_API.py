...
def get_symbols(expiry, name, strike, ins_type):
    ...
    lst_b = [
        ...
        if num["expiry"] == expiry
        and num["strike"] == strike
        and num["instrument_type"] == ins_type
        and num["name"] == name
    ]
...
def place_order_aliceblue(i, q, b_or_s="BUY"):
    ...
    b_or_s = b_or_s.upper()
    try:
        od = alice.place_order(
            transaction_type=(
                TransactionType.Sell if b_or_s == "SELL" else TransactionType.Buy
            ),
            instrument=i,
            quantity=q,
...
def createshortstraddlebnf():
    ...
    try:
        next_thursday_expiry = date.today() + relativedelta(weekday=WE(+1))
        # next_thursday_expiry = next_thursday_expiry+timedelta(days=-1)

        ce = alice.get_instrument_for_fno(
            exch="NFO",
            symbol="BANKNIFTY",
            expiry_date=next_thursday_expiry.strftime("%Y-%m-%d"),
            is_fut=False,
            strike=strike_ce,
            is_CE=True,
        )
    ...
    print(
        f"order for {symbol_ce['TSymbl']} - CE sell placed at {str(symbol_ce['LTP'])} "
    )
    logging.info(
        f"order for {symbol_ce['TSymbl']} - CE sell placed at {str(symbol_ce['LTP'])} "
    )
    ...
    try:
        next_thursday_expiry = datetime.combine(
            next_thursday_expiry, datetime.min.time()
        )
...
def updatecombinedpremiums():
    ...
    for key, value in bnfshortstraddle_dict.items():
        if value is not None:
            ...
            logging.info(
                "latest premium for {%s} - CE sell value now  - {%s} - placed at - {%s}",
                symbol_ce["TSymbl"],
                str(symbol_ce["LTP"]),
                str(key),
            )
            logging.info(
                "latest premium for {%s} - PE sell value now - {%s} - placed at - {%s}",
                symbol_pe["TSymbl"],
                str(symbol_pe["LTP"]),
                str(key),
            )
...
def checkslhit(ce, pe, key):
    ...
    if item.current_ce_ltp >= slprice:
        ...
        print(
            "Stop Loss Order for {%s} - CE sell placed at -{%s} ",
            ce["TSymbl"],
            str(ce["LTP"]),
        )
        logging.info(
    ...
    if item.current_pe_ltp >= slprice:
        ...
        print(
            "Stop Loss Order for {%s} - PE sell placed at -{%s} ",
            pe["TSymbl"],
            str(pe["LTP"]),
        )
        logging.info(
        ...
        for index, row in listoftrades.iterrows():
            ...
            if row["ProductType"] == "sell" or row["ProductType"] == "intradaysell":
                if row["TpHit"] == True:
                    ...
                    trading_price = getlasttradedprice(
                        row["Strike"], row["Expiry"].date().strftime("%Y-%m-%d"), is_ce
                    )
...
def get_traded_records(tradeddate, is_cumulative):
    ...
    list_trades =list(coll.find({})) 
    listoftrades = pd.DataFrame(list_trades)

    return listoftrades,list_trades
...
def check_sl_pt():
    ...
    listoftrades,_ = get_traded_records(collectionname, True)
    for index, row in listoftrades.iterrows():
        ...
        if "BANKNIFTY" in row["TradingSymbol"]:
            ...
            trading_price = getlasttradedprice(
                row["Strike"],
                datetime.datetime.strptime(row["Expiry"], "%d%b%y")
                .date()
                .strftime("%Y-%m-%d"),
                is_ce,
            )
        ...
        if row["SlHit"] == False and row["TpHit"] == False:
            ...
            if row["OrderType"] == "SELL":
                sl_val = traded_price + traded_price * 0.2
            ...
            if row["OrderType"] == "SELL":
                ...
                mtmlossprofit = mtmlossprofit + (
                    traded_price * row["Qty"] - trading_price * row["Qty"]
                )
            else:
